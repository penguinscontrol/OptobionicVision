	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 3);
 	...
 	ivoc_help("help ?1 opto C:/Users/Radu/Documents/GitHub/Spike2Scripts/Final_Project/modandhocfiles/opto.mod\n");
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
    gchr2 = ( No1 / N + ( go2 / go1 ) * No2 / N ) * ( 1.0 - exp ( - ( v + 8.0 ) / U0 ) ) / ( ( v + 8.0 ) / U1 ) ;
    ina = gchr2 * ( v + 8.0 ) ;
   }
 _current += ina;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 58 in file opto.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
